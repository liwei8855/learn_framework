...
/**
 ...
 RKResponseDescriptor对象描述了适用于HTTP响应的对象映射配置。 通过指定在反序列化响应主体上执行对象映射时要使用的“ RKMapping”对象以及适合该映射的URL路径模式，键路径和状态代码来定义响应描述符。 路径模式是SOCKit`SOCPattern`字符串，将与加载了要映射的响应的请求的URL匹配。 如果路径模式为nil，则认为响应描述符适用于从任何URL加载的响应
 关键路径指定了反序列化的响应体内数据的位置，映射适用于此位置。 如果为零，则认为映射适用于整个响应主体。 状态代码指定了一组HTTP响应状态代码，该映射适用于该状态。 通常将响应描述符约束到HTTP成功状态代码类（状态代码在200-299范围内）。 错误响应的对象映射可以通过配置响应描述符来配置以处理客户端错误状态代码类（400-499范围内的状态代码）。 RKResponseDescriptor的实例是不可变的
